...
describe WhileStatement do
  let(:while_statement) { WhileStatement.new(token: 'token', condition: condition, consequence: consequence) }

  describe '#to_h / to_xml' do
    subject(:hash) { while_statement.to_h }
    subject(:xml) { while_statement.to_xml }
    ...
    context 'without consequence' do
      ...
      it do
        expected ='<whileStatement><keyword>while</keyword><symbol>(</symbol><symbol>(</symbol><expression><term><identifier>a</identifier></term><symbol>&lt;</symbol><term><integerConstant>2</integerConstant></term></expression><symbol>)</symbol><symbol>{</symbol><statements></statements><symbol>}</symbol></whileStatement>'
    ...
    context 'with consequence' do
      ...
      context 'wuthout altenative' do
        ...
        it do
          expected ='<whileStatement><keyword>while</keyword><symbol>(</symbol><symbol>(</symbol><expression><term><identifier>a</identifier></term><symbol>&lt;</symbol><term><integerConstant>2</integerConstant></term></expression><symbol>)</symbol><symbol>{</symbol><statements><letStatement><keyword>let</keyword><identifier>test1</identifier><symbol>=</symbol><expression><term><integerConstant>1</integerConstant></term><symbol>+</symbol><term><integerConstant>2</integerConstant></term></expression><symbol>;</symbol></letStatement><returnStatement><keyword>return</keyword><expression><term><identifier>test1</identifier></term></expression><symbol>;</symbol></returnStatement></statements><symbol>}</symbol></whileStatement>'
          expect(xml).to eq expected
        end
      end
    end
